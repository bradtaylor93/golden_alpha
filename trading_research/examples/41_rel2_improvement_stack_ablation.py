...
class Config:
    ...
    dynamic_cluster_abs_weight_cap_stressed: float = 0.18
    dynamic_cluster_abs_weight_cap_calm: float = 0.30
...
EXPERIMENTS: tuple[ExperimentSpec, ...] = (
    # Baseline winner to beat.
    ExperimentSpec(name="baseline_nhf5", use_reentry_cooldown=True),
    # 1) regime-aware exposure throttle
    ExperimentSpec(
        name="imp1_regime_throttle",
        use_reentry_cooldown=True,
        use_dynamic_gross_target=True,
    ),
    # 2) tail-loss control: vol-normalized trail + hard stop
    ExperimentSpec(
        name="imp2_tail_control",
        use_reentry_cooldown=True,
        use_vol_norm_trail=True,
        use_hard_stop_loss=True,
    ),
    # 3) adaptive cooldown by regime
    ExperimentSpec(
        name="imp3_adaptive_cooldown",
        use_reentry_cooldown=True,
        use_adaptive_cooldown=True,
    ),
    # 4) expected-edge floor (implemented via dynamic edge floor)
    ExperimentSpec(
        name="imp4_edge_floor",
        use_reentry_cooldown=True,
        use_dynamic_edge_floor=True,
        use_custom_edge_threshold=True,
        custom_base_edge_threshold=0.015,
        custom_additional_edge_threshold=0.015,
    ),
    # 5) dynamic cluster caps (tighter in stressed regimes)
    ExperimentSpec(
        name="imp5_dynamic_cluster_caps",
        use_reentry_cooldown=True,
        use_dynamic_cluster_caps=True,
    ),
    # Key combinations.
    ExperimentSpec(
        name="combo_12_regime_tail",
        use_reentry_cooldown=True,
        use_dynamic_gross_target=True,
        use_vol_norm_trail=True,
        use_hard_stop_loss=True,
    ...
    ExperimentSpec(
        name="combo_123_regime_tail_adaptivecd",
        use_reentry_cooldown=True,
        use_dynamic_gross_target=True,
        use_vol_norm_trail=True,
        use_hard_stop_loss=True,
    ...
    for ts in all_dates:
        ...
        for c in cands:
            ...
            for j in range(start_idx, hard_end + 1):
                ...
                if exp.use_hard_stop_loss and signed_ret <= -float(cfg.hard_stop_loss_pct):
                    exit_idx = j
