"""Low-hanging-fruit upgrades on rel2_top8 best policy.

Tests five incremental improvements (one-at-a-time) on top of:
- rel2_top8 entry
- trend-conditioned exits: bull hold 98, bear hold 42, bear trailing 10%
- confidence-weighted sizing baseline

Improvements:
1) beta hedge v2 (conditional + capped),
2) turnover-aware sizing (weight smoothing),
3) edge floor trade filter,
4) confidence-decay early exit,
5) sector-like exposure caps via correlation clusters.
"""
...
class Config:
    ...
    min_calibration_trades: int = 300
    beta_window_days: int = 63
    beta_activate_threshold: float = 0.25
    beta_cap_v2: float = 0.35
    turnover_smoothing_lambda: float = 0.20
    edge_floor_threshold: float = 0.015
    decay_min_hold_days: int = 20
    decay_ratio: float = 0.60
...
class ExperimentSpec:
    name: str
    use_beta_hedge_v2: bool = False
    use_turnover_smoothing: bool = False
    use_edge_floor_filter: bool = False
    use_conf_decay_exit: bool = False
    use_cluster_caps: bool = False


EXIT_SPEC = ExitSpec(bull_hold_days=98, bear_hold_days=42, bear_trailing_stop_pct=0.10)

EXPERIMENTS: tuple[ExperimentSpec, ...] = (
    ExperimentSpec(name="base_confidence"),
    ExperimentSpec(name="lhf1_beta_hedge_v2", use_beta_hedge_v2=True),
    ExperimentSpec(name="lhf2_turnover_smoothing", use_turnover_smoothing=True),
...
def _build_trades(
    ...
    exit_spec: ExitSpec,
    fold_id: str,
    use_edge_floor_filter: bool,
    use_conf_decay_exit: bool,
) -> pd.DataFrame:
    ...
    for asset, g in test_panel.groupby("asset", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            confidence = _confidence_from_row(r, cfg)
            signal_strength = float(max(1e-6, rank - cfg.top_quantile))
            edge_proxy = signal_strength * confidence
            if use_edge_floor_filter and edge_proxy < cfg.edge_floor_threshold:
                continue
            ...
            for j in range(start_idx, hard_end + 1):
                ...
                if use_conf_decay_exit and (j - start_idx) >= cfg.decay_min_hold_days:
                    if ts_j in g_by_ts.index:
                        cur = g_by_ts.loc[ts_j]
                        cur_conf = _confidence_from_row(cur, cfg)
                        if cur_conf < (entry_conf * cfg.decay_ratio):
                            exit_idx = j
                            exit_reason = "confidence_decay_exit"
                            break
...
def _weights_from_active(
    ...
    for _, r in by_asset.iterrows():
        a_name = str(r["asset"])
        if a_name in w.index:
            w.loc[a_name] = float(r["weight"])
    ...
    if t.empty:
        ...
        for i in range(1, len(dates)):
            ...
            if exp.use_beta_hedge_v2:
                ...
                high_vol = bool(np.isfinite(vol_now) and np.isfinite(train_spy_vol_median) and vol_now > train_spy_vol_median)
                is_bear = bool(float(spy_up.loc[ts_i]) <= 0.5) if ts_i in spy_up.index else False
                if len(hist_strat) >= cfg.beta_window_days and len(hist_spy) >= cfg.beta_window_days and (high_vol or is_bear):
                    ...
                    if vx > 1e-10:
                        ...
                        if abs(beta_est) >= cfg.beta_activate_threshold:
                            hedge_beta = float(np.clip(beta_est, 0.0, cfg.beta_cap_v2))
...
def _simulate_buy_hold_all_fold(test_panel: pd.DataFrame, spy_df: pd.DataFrame, cfg: Config, fold_id: str) -> pd.DataFrame:
    ...
    out = pd.DataFrame({"timestamp": rets.index, "net_return": net, "turnover": turnover, "gross_exposure": 1.0, "hedge_beta": 0.0})
    spy = spy_df.copy()
    spy["timestamp"] = pd.to_datetime(spy["timestamp"], utc=True, errors="coerce")
    spy = spy.dropna(subset=["timestamp"]).sort_values("timestamp")
...
def _portfolio_metrics(daily: pd.DataFrame, trades: pd.DataFrame) -> dict[str, float]:
    ...
    sr_n = float((daily["net_return"].mean() / daily["net_return"].std(ddof=1)) * math.sqrt(252.0)) if len(daily) > 1 and float(daily["net_return"].std(ddof=1)) > 0 else float("nan")
    sr_a = float((daily["alpha_return"].mean() / daily["alpha_return"].std(ddof=1)) * math.sqrt(252.0)) if len(daily) > 1 and float(daily["alpha_return"].std(ddof=1)) > 0 else float("nan")
...
def main() -> None:
    out_root = Path("trading_research/examples/_output/36_rel2_top8_low_hanging_fruits")
    ...
    for test_year in years[cfg.min_train_years :]:
        ...
        for exp in EXPERIMENTS:
            trades = _build_trades(
                ...
                exit_spec=EXIT_SPEC,
                fold_id=fold_id,
                use_edge_floor_filter=exp.use_edge_floor_filter,
                use_conf_decay_exit=exp.use_conf_decay_exit,
            )
    ...
    for name in strategy_names:
        daily_all = pd.concat([d for d in all_daily_by_strategy[name] if not d.empty], ignore_index=True) if all_daily_by_strategy[name] else pd.DataFrame()
    ...
    base = overall_df[overall_df["strategy"] == "base_confidence"]
