"""Trend-conditioned exit tuning for rel2_tight_top8 vs buy-and-hold.

Entry logic is unchanged (rel2_tight_top8):
- score rank in top 8%
- relative strength vs SPY >= +2%

Only exits change using simple market-trend conditioning from SPY 200D MA.
"""
...
class ExitSpec:
    ...
    bull_hold_days: int
    bear_hold_days: int
    bull_trailing_stop_pct: float | None = None
    bear_trailing_stop_pct: float | None = None


EXIT_SPECS: tuple[ExitSpec, ...] = (
    ExitSpec(name="rel2_top8_fixed63", bull_hold_days=63, bear_hold_days=63),
    ExitSpec(name="rel2_top8_trend_h84_b42", bull_hold_days=84, bear_hold_days=42),
    ExitSpec(
        name="rel2_top8_trend_h84_b42_bear_trail10",
        bull_hold_days=84,
        bear_hold_days=42,
    ...
    ExitSpec(
        name="rel2_top8_trend_h84_b63_bear_trail10",
        bull_hold_days=84,
        bear_hold_days=63,
        bear_trailing_stop_pct=0.10,
    ),
...
def _build_trades_for_exit(
    ...
    for asset, g in test_panel.groupby("asset", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            trail = exit_spec.bull_trailing_stop_pct if spy_up else exit_spec.bear_trailing_stop_pct

            start_idx = sig_idx + 1
            hard_end = min(sig_idx + int(hold_days), n_dates - 1)
            ...
            if trail is not None:
                for j in range(start_idx, hard_end + 1):
                    ...
                    if (peak_signed - signed_ret) >= float(trail):
                        exit_idx = j
                        exit_reason = f"{'bull' if spy_up else 'bear'}_trail_{int(round(float(trail) * 100)):02d}"
                        break

            rows.append(
                {
                    "fold_id": fold_id,
                    "strategy": exit_spec.name,
                    "asset": str(asset),
                    ...
                    "entry_regime": "bull" if spy_up else "bear",
...
def _simulate_buy_hold_all_fold(
    ...
    gross = rets.reindex(columns=w.index).fillna(0.0).dot(w)
    bh = pd.DataFrame({"timestamp": rets.index, "net_return": gross.to_numpy()})
    bh["turnover"] = 0.0
...
def main() -> None:
    out_root = Path("trading_research/examples/_output/33_rel2_top8_trend_exit_vs_buyhold")
    ...
    strategy_names = [s.name for s in EXIT_SPECS] + ["buy_hold_all"]
    all_daily_by_strategy: dict[str, list[pd.DataFrame]] = {k: [] for k in strategy_names}
    all_trades_by_strategy: dict[str, list[pd.DataFrame]] = {k: [] for k in strategy_names}
    ...
    for test_year in years[cfg.min_train_years :]:
        ...
        for exit_spec in EXIT_SPECS:
            trades = _build_trades_for_exit(test, cfg, exit_spec=exit_spec, fold_id=fold_id)
            daily, trades_eval = _simulate_fold(
                ...
                cfg=cfg,
                fold_id=fold_id,
                strategy_name=exit_spec.name,
            )
            if daily.empty:
                continue
            all_daily_by_strategy[exit_spec.name].append(daily)
            all_trades_by_strategy[exit_spec.name].append(trades_eval)
            fold_rows.append(
                {
                    "strategy": exit_spec.name,
                    "fold_id": fold_id,
    ...
    baseline = overall_df[overall_df["strategy"] == "rel2_top8_fixed63"]
    ...
    if not baseline.empty and best:
        ...
        uplift = {
            ...
            "delta_annual_return_vs_fixed63": float(best["annual_return"] - b["annual_return"]),
            "delta_cagr_vs_fixed63": float(best["cagr"] - b["cagr"]),
            "delta_net_sharpe_vs_fixed63": float(best["annualized_sharpe_net"] - b["annualized_sharpe_net"]),
            "delta_alpha_sharpe_vs_fixed63": float(best["annualized_sharpe_alpha"] - b["annualized_sharpe_alpha"]),
            "delta_maxdd_vs_fixed63": float(best["max_drawdown"] - b["max_drawdown"]),
            "delta_n_trades_vs_fixed63": float(best["n_trades"] - b["n_trades"]),
        }
