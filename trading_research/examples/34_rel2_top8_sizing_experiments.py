"""Volatility-aware and confidence-aware sizing experiments for rel2_top8.

This study keeps the signal/exit logic fixed and tests only position sizing:
- equal sizing (baseline)
- inverse-volatility sizing
- confidence-weighted sizing
- inverse-volatility + confidence sizing

Benchmark included:
- buy-and-hold all-assets equal-weight portfolio
"""
...
class SizingSpec:
    name: str
    mode: str  # equal | vol_inv | confidence | vol_inv_confidence
...
SIZING_SPECS: tuple[SizingSpec, ...] = (
    SizingSpec(name="top8_trend_best_size_equal", mode="equal"),
    SizingSpec(name="top8_trend_best_size_vol_inv", mode="vol_inv"),
    SizingSpec(name="top8_trend_best_size_confidence", mode="confidence"),
    SizingSpec(name="top8_trend_best_size_vol_inv_conf", mode="vol_inv_confidence"),
)
...
def _weights_from_active(active: pd.DataFrame, cfg: Config, assets: list[str], sizing: SizingSpec) -> pd.Series:
    ...
    if sizing.mode == "equal":
        a["raw"] = a["signal_strength"]
    elif sizing.mode == "vol_inv":
        a["raw"] = a["signal_strength"] / a["entry_vol_21"]
    elif sizing.mode == "confidence":
        a["raw"] = a["signal_strength"] * a["confidence_score"]
    elif sizing.mode == "vol_inv_confidence":
        a["raw"] = a["signal_strength"] * a["confidence_score"] / a["entry_vol_21"]
    else:
    ...
    for asset, g in test_panel.groupby("asset", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            for j in range(start_idx, hard_end + 1):
                ...
                trail = exit_spec.bear_trailing_stop_pct if not spy_up else None
                if trail is not None and (peak_signed - signed_ret) >= float(trail):
                    exit_idx = j
                    exit_reason = f"bear_trailing_{int(round(trail * 100)):02d}"
                    break

            # Confidence uses both tail strength and rel-strength excess.
            ...
            confidence = 0.5 + 1.5 * (0.5 * rank_conf + 0.5 * rel_conf)
...
def _simulate_fold(
    ...
    if t.empty:
        ...
        for i in range(1, len(dates)):
            ...
            w = _weights_from_active(active, cfg, assets, sizing=sizing)
...
def main() -> None:
    out_root = Path("trading_research/examples/_output/34_rel2_top8_sizing_experiments")
    ...
    strategy_names = [s.name for s in SIZING_SPECS] + ["buy_hold_all"]
    ...
    for test_year in years[cfg.min_train_years :]:
        ...
        for sizing in SIZING_SPECS:
            daily, trades_eval = _simulate_fold(
                ...
                cfg=cfg,
                fold_id=fold_id,
                strategy_name=sizing.name,
                sizing=sizing,
            )
            if daily.empty:
                continue
            all_daily_by_strategy[sizing.name].append(daily)
            all_trades_by_strategy[sizing.name].append(trades_eval)
            fold_rows.append(
                {
    ...
    base = overall_df[overall_df["strategy"] == "top8_trend_best_size_equal"]
    ...
    if not base.empty and best:
        ...
        uplift = {
            ...
            "delta_annual_return_vs_equal": float(best["annual_return"] - b["annual_return"]),
            "delta_cagr_vs_equal": float(best["cagr"] - b["cagr"]),
            "delta_net_sharpe_vs_equal": float(best["annualized_sharpe_net"] - b["annualized_sharpe_net"]),
            "delta_alpha_sharpe_vs_equal": float(best["annualized_sharpe_alpha"] - b["annualized_sharpe_alpha"]),
            "delta_maxdd_vs_equal": float(best["max_drawdown"] - b["max_drawdown"]),
            "delta_n_trades_vs_equal": float(best["n_trades"] - b["n_trades"]),
        }
    ...
    summary = {
        ...
        "sizing_specs": [s.__dict__ for s in SIZING_SPECS],
        "uplift": uplift,
    }
