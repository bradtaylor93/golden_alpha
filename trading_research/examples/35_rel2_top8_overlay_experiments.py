"""Run experiments 1-4 on rel2_top8 trend-exit policy.

Experiments requested:
1) beta-hedge overlay,
2) correlation-aware risk budgeting,
3) regime-aware exposure throttle,
4) calibrated confidence sizing.

All experiments keep the same entry + exit policy and differ only by overlay logic.
"""
...
class Config:
    ...
    # experiment 1: beta-hedge overlay
    beta_window_days: int = 63
    beta_cap: float = 1.5
    hedge_cost_bps_per_side: float = 2.0
    # experiment 2: corr-aware risk budget
    corr_window_days: int = 63
    corr_penalty: float = 1.0
    # experiment 3: regime throttle (portfolio-level exposure)
    throttle_scale_good: float = 1.00
    throttle_scale_mixed: float = 0.85
    throttle_scale_bad: float = 0.65
    # experiment 4: confidence calibration
    min_calibration_trades: int = 120
...
class ExitSpec:
    ...
    bull_hold_days: int
    bear_hold_days: int
    bear_trailing_stop_pct: float | None = None
...
class ExperimentSpec:
    name: str
    use_beta_hedge: bool
    use_corr_budget: bool
    use_regime_throttle: bool
...
EXPERIMENTS: tuple[ExperimentSpec, ...] = (
    ExperimentSpec(
        name="exp0_baseline_confidence",
        use_beta_hedge=False,
        use_corr_budget=False,
        use_regime_throttle=False,
        use_calibrated_confidence=False,
    ),
    ExperimentSpec(
        name="exp1_beta_hedge_overlay",
        use_beta_hedge=True,
        use_corr_budget=False,
        use_regime_throttle=False,
        use_calibrated_confidence=False,
    ...
    ExperimentSpec(
        name="exp2_corr_risk_budget",
        use_beta_hedge=False,
        use_corr_budget=True,
        use_regime_throttle=False,
        use_calibrated_confidence=False,
    ),
    ExperimentSpec(
        name="exp3_regime_throttle",
        use_beta_hedge=False,
        use_corr_budget=False,
        use_regime_throttle=True,
        use_calibrated_confidence=False,
    ),
    ExperimentSpec(
        name="exp4_calibrated_confidence",
        use_beta_hedge=False,
        use_corr_budget=False,
        use_regime_throttle=False,
...
def _load_universe_170() -> tuple[str, ...]:
    src = Path("trading_research/examples/34_rel2_top8_sizing_experiments.py")
    if not src.exists():
        raise FileNotFoundError("Cannot load universe source from examples/34.")
    text = src.read_text(encoding="utf-8")
    tickers = re.findall(r'^\s*"([A-Z][A-Z0-9\-]{0,9})",\s*$', text, flags=re.MULTILINE)
    if len(tickers) < 120:
        raise ValueError("Parsed universe looks too small; refusing to proceed.")
...
def _build_panel(bars: pd.DataFrame, spy: pd.DataFrame, cfg: Config) -> pd.DataFrame:
    ...
    frame["vol_21"] = (
        g["ret_1d"].rolling(cfg.vol_window_days, min_periods=cfg.vol_window_days).std().reset_index(level=0, drop=True)
    )
...
def _build_trades(test_panel: pd.DataFrame, cfg: Config, exit_spec: ExitSpec, fold_id: str) -> pd.DataFrame:
    ...
    for asset, g in test_panel.groupby("asset", sort=False):
        ...
        for _, r in g.iterrows():
            ...
            start_idx = sig_idx + 1
            hold_days = exit_spec.bull_hold_days if spy_up else exit_spec.bear_hold_days
            ...
            for j in range(start_idx, hard_end + 1):
                ...
                if (not spy_up) and exit_spec.bear_trailing_stop_pct is not None:
                    trail = float(exit_spec.bear_trailing_stop_pct)
                    if (peak_signed - signed_ret) >= trail:
                        exit_idx = j
                        exit_reason = f"bear_trailing_{int(round(trail * 100)):02d}"
                        break
            ...
            raw_conf = 0.5 + 1.5 * (0.5 * rank_conf + 0.5 * rel_conf)
...
def _evaluate_trade_returns(panel: pd.DataFrame, trades: pd.DataFrame, cfg: Config) -> pd.Series:
    if trades.empty:
        return pd.Series(dtype=float)
    close_wide = panel.pivot(index="timestamp", columns="asset", values="close").sort_index().ffill()
    dates = close_wide.index.to_list()
    ...
    for _, r in trades.iterrows():
        ...
        if asset not in close_wide.columns:
            continue
        sidx = int(float(r["start_idx"]))
        eidx = int(float(r["end_idx"]))
        if sidx < 0 or eidx >= len(dates) or sidx >= eidx:
            continue
        entry_px = float(close_wide.loc[dates[sidx], asset])
        exit_px = float(close_wide.loc[dates[eidx], asset])
        sign = float(r["signal_sign"])
        gross = sign * (exit_px / entry_px - 1.0)
        out.append(float(gross - rt_cost))
    return pd.Series(out, dtype=float)


def _fit_confidence_calibrator(train_panel: pd.DataFrame, cfg: Config, exit_spec: ExitSpec) -> dict[str, float] | None:
    train_trades = _build_trades(train_panel, cfg=cfg, exit_spec=exit_spec, fold_id="train")
    if len(train_trades) < cfg.min_calibration_trades:
        return None
    conf = pd.to_numeric(train_trades["raw_confidence"], errors="coerce")
    ret = _evaluate_trade_returns(train_panel, train_trades, cfg)
    n = int(min(len(conf.dropna()), len(ret.dropna())))
    if n < cfg.min_calibration_trades:
        return None
    x = conf.dropna().to_numpy(dtype=float)[:n]
    y = ret.dropna().to_numpy(dtype=float)[:n]
    ...
    if denom <= 1e-9:
        return None
    slope = float(np.dot(xz, y_center) / denom)
    pred = slope * xz
    scale = float(np.quantile(np.abs(pred), 0.90))
    if scale <= 1e-10:
        return None
    return {"x_mean": x_mean, "x_std": x_std, "slope": slope, "scale": scale}


def _apply_confidence_calibration(trades: pd.DataFrame, model: dict[str, float] | None) -> pd.DataFrame:
    out = trades.copy()
    base = pd.to_numeric(out["raw_confidence"], errors="coerce").fillna(1.0).clip(lower=0.25, upper=3.0)
    if model is None:
        out["confidence_score"] = base
        return out
    xz = (base - float(model["x_mean"])) / max(1e-9, float(model["x_std"]))
    pred = float(model["slope"]) * xz
    mult = (1.0 + (pred / max(1e-10, float(model["scale"]))) * 0.5).clip(lower=0.5, upper=1.5)
    out["confidence_score"] = (base * mult).clip(lower=0.25, upper=3.0)
    return out
...
def _weights_from_active(
    ...
    assets: list[str],
    use_corr_budget: bool,
    corr_mat: pd.DataFrame | None,
    gross_target_day: float,
) -> pd.Series:
    ...
    a = active.copy()
    a["signal_strength"] = pd.to_numeric(a["signal_strength"], errors="coerce")
    a["signal_sign"] = pd.to_numeric(a["signal_sign"], errors="coerce")
    a["entry_vol_21"] = pd.to_numeric(a["entry_vol_21"], errors="coerce").clip(lower=cfg.vol_floor)
    a["confidence_score"] = pd.to_numeric(a["confidence_score"], errors="coerce").clip(lower=0.25, upper=3.0)
    a = a.dropna(subset=["signal_strength", "signal_sign", "entry_vol_21", "confidence_score"])
    if a.empty:
        return w

    # Baseline confidence sizing from prior winning sizing experiment.
    ...
    if use_corr_budget and corr_mat is not None and not corr_mat.empty:
        assets_active = [str(x) for x in by_asset["asset"]]
        corr_sub = corr_mat.reindex(index=assets_active, columns=assets_active)
        corr_sub = corr_sub.fillna(0.0)
        # Penalize crowded correlated names.
        for i, asset in enumerate(assets_active):
            row = np.abs(corr_sub.iloc[i].to_numpy(dtype=float))
            if len(row) > 1:
                mean_abs_corr = float((np.sum(row) - 1.0) / (len(row) - 1))
            else:
                mean_abs_corr = 0.0
            penalty = 1.0 + cfg.corr_penalty * max(0.0, mean_abs_corr)
            by_asset.loc[by_asset["asset"] == asset, "raw_signed"] = (
                by_asset.loc[by_asset["asset"] == asset, "raw_signed"] / penalty
            )

    denom = float(np.sum(np.abs(by_asset["raw_signed"])))
    if denom <= 0.0:
        return w
    by_asset["weight"] = (by_asset["raw_signed"] / denom) * gross_target_day
...
def _simulate_fold(
    ...
    if t.empty:
        ...
        t = t.dropna(
            subset=["start_idx", "end_idx", "asset", "signal_sign", "signal_strength", "entry_vol_21", "confidence_score"]
        )
        ...
        for i in range(1, len(dates)):
            ...
            regime_scale = 1.0
            if exp.use_regime_throttle:
                up = float(spy_up.loc[ts_i]) if ts_i in spy_up.index else 0.0
                vol_now = float(spy_vol21.loc[ts_i]) if ts_i in spy_vol21.index else float("nan")
                low_vol = bool(np.isfinite(vol_now) and np.isfinite(train_spy_vol_median) and vol_now <= train_spy_vol_median)
                if up > 0.5 and low_vol:
                    regime_scale = cfg.throttle_scale_good
                elif (up > 0.5) ^ low_vol:
                    regime_scale = cfg.throttle_scale_mixed
                else:
                    regime_scale = cfg.throttle_scale_bad

            corr_mat: pd.DataFrame | None = None
            if exp.use_corr_budget:
                start = max(1, i - cfg.corr_window_days)
                hist = rets.iloc[start:i]
                if len(hist) >= 20:
                    corr_mat = hist.corr().replace([np.inf, -np.inf], np.nan).fillna(0.0)

            gross_target_day = cfg.gross_target * regime_scale
            w = _weights_from_active(
                active=active,
                cfg=cfg,
                assets=assets,
                use_corr_budget=exp.use_corr_budget,
                corr_mat=corr_mat,
                gross_target_day=gross_target_day,
            )
            ...
            if exp.use_beta_hedge:
                ...
                bench_coeff = max(0.0, 1.0 - hedge_beta)
                prev_hedge = hedge_beta

            alpha = net - bench_coeff * spy_r
            rows.append(
                {
...
def _simulate_buy_hold_all_fold(test_panel: pd.DataFrame, spy_df: pd.DataFrame, cfg: Config, fold_id: str) -> pd.DataFrame:
    ...
    out = pd.DataFrame({"timestamp": rets.index, "net_return": net, "turnover": turnover, "gross_exposure": 1.0})
    spy = spy_df.copy()
    spy["timestamp"] = pd.to_datetime(spy["timestamp"], utc=True, errors="coerce")
    spy = spy.dropna(subset=["timestamp"]).sort_values("timestamp")
...
def _portfolio_metrics(daily: pd.DataFrame, trades: pd.DataFrame) -> dict[str, float]:
    ...
    return {
        ...
        "avg_regime_scale": float(pd.to_numeric(daily.get("regime_scale"), errors="coerce").mean()),
    }


def main() -> None:
    out_root = Path("trading_research/examples/_output/35_rel2_top8_overlay_experiments")
    ...
    for test_year in years[cfg.min_train_years :]:
        ...
        calibrator = _fit_confidence_calibrator(train, cfg=cfg, exit_spec=EXIT_SPEC)
        train_spy_vol_median = float(pd.to_numeric(train["spy_vol_21"], errors="coerce").median())

        base_trades = _build_trades(test, cfg=cfg, exit_spec=EXIT_SPEC, fold_id=fold_id)
        if base_trades.empty:
            continue

        for exp in EXPERIMENTS:
            trades = base_trades.copy()
            if exp.use_calibrated_confidence:
                trades = _apply_confidence_calibration(trades, calibrator)
            else:
                trades = trades.copy()
                trades["confidence_score"] = pd.to_numeric(trades["raw_confidence"], errors="coerce").fillna(1.0)
    ...
    base = overall_df[overall_df["strategy"] == "exp0_baseline_confidence"]
    ...
    if not base.empty and best:
        ...
        uplift = {
            ...
            "delta_annual_return_vs_baseline": float(best["annual_return"] - b["annual_return"]),
            ...
            "delta_maxdd_vs_baseline": float(best["max_drawdown"] - b["max_drawdown"]),
            "delta_n_trades_vs_baseline": float(best["n_trades"] - b["n_trades"]),
        }
    ...
    summary = {
        ...
        "config": {
            ...
            "hedge_one_way_cost_return": _hedge_one_way_cost_return(cfg),
        },
        "exit_spec": EXIT_SPEC.__dict__,
